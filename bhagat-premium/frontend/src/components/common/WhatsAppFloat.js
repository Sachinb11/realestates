...
const WA_NUMBER  = process.env.NEXT_PUBLIC_WHATSAPP || '918975127927';
const DEFAULT_MSG = "Hi, I'm interested in a property from Bhagat Estates. Please share details.";

export default function WhatsAppFloat({ propertyTitle = '' }) {
  const [visible, setVisible] = useState(false);
  const [showLabel, setShowLabel] = useState(false);

  useEffect(() => {
    const t1 = setTimeout(() => setVisible(true), 1800);
    const t2 = setTimeout(() => setShowLabel(true), 3000);
  ...
  return (
    <div className={`${styles.wrap} ${visible ? styles.visible : ''}`}>
      {showLabel && <span className={styles.label}>Chat with us!</span>}
